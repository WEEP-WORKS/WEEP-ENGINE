...
UIFontsSystem::UIFontsSystem() //UIMain * _ui_main
{
	//ui_main = _ui_main;
}
...
ResourceTexture UIFontsSystem::LoadText(const char * text, UIFont * font, uint size)
{
	...
	return SuStoTexture(id, vec2(bitmap_x, bitmap_y));
}
...
UITexture::UITexture(const UITexture & copy)
{
	id = copy.id;
	size = copy.size;
}

UITexture::UITexture(uint _texture_id, vec2 _texture_size)
{
	id = _texture_id;
	size = _texture_size;
}
